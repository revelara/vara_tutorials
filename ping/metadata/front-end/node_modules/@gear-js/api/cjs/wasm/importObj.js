...
var importObj = (memory, showDebug, inputValue, timestamp, block_height, replyFn = () => { }) => ({
    env: {
        ...
        gr_debug: (payload, len) => {
            if (showDebug) {
                console.debug('[GR_DEBUG]', new types.TypeRegistry()
                    .createType('String', new Uint8Array(memory.buffer.slice(payload, payload + len)))
                    .toHuman());
            }
        },
        gr_error: (error, len) => {
            console.error('[GR_ERROR]', new types.TypeRegistry().createType('String', new Uint8Array(memory.buffer.slice(error, error + len))).toHuman());
        },
        gr_origin: () => { },
        gr_pay_program_rent: () => { },
        gr_status_code: () => { },
        ...
        gr_size: (size_ptr) => {
            assert.notStrictEqual(inputValue, undefined, 'Input value not found');
            const len = new types.TypeRegistry().createType('u32', inputValue.byteLength).toU8a();
